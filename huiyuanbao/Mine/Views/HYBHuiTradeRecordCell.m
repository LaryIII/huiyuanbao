...
@interface HYBHuiTradeRecordCell()

@end

@implementation HYBHuiTradeRecordCell
...
- (void)setHuiTradeRecord:(HYBHuiTradeRecord *)huiTradeRecord
{
    _huiTradeRecord = huiTradeRecord;
    int padding = 8;
    ...
    storeTitle.text = _huiTradeRecord.game_name;//@"消费";
    ...
    time.text = _huiTradeRecord.order_time;//@"02-30 18:00";
    ...
    NSString *statusText = @"";
    switch ([_huiTradeRecord.status integerValue]) {
        case 0:
            statusText = @"处理中";
            break;
        case 1:
    ...
    youvalue.text = [NSString stringWithFormat:@"￥%@", _huiTradeRecord.rebate_money];//@"￥0.00";
    ...
    alivalue.text = [NSString stringWithFormat:@"￥%@", _huiTradeRecord.total_money];//@"￥3.00";
...
-(void)clickAction{
    if (self.delegate && [self.delegate respondsToSelector:@selector(gotoHuiTradeRecordDetail:withHuiTradeRecord:)]) {
        [self.delegate gotoHuiTradeRecordDetail:self withHuiTradeRecord:_huiTradeRecord];
    }
}
@end
