...
@implementation HYBFirstStoreCell
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UIImage *image = [UIImage imageNamed:@"first_store"];
        [icon setImage:image];
...
{
    return CGSizeMake(containerWidth, 158.0f);
}

-(void)clickAction{
    if (self.delegate && [self.delegate respondsToSelector:@selector(gotoFirstStoreDetail:withStore:)]) {
        [self.delegate gotoFirstStoreDetail:self withStore:_store];
    }
}
