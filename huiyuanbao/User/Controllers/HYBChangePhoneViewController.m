//
//  HYBChangePhoneViewController.m
//  huiyuanbao
//
...
@property (nonatomic, strong) UITextField *psdfield2;
...
- (void)viewDidLoad {
    ...
    UIImageView *psdicon2 = UIImageView.new;
    psdicon2.image = [UIImage imageNamed:@"u_psw"];
    [self.view addSubview:psdicon2];
    [psdicon2 makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(lineview3.bottom).offset((inputHeight-25.0f)/2);
        make.left.equalTo(superview.left).offset(15);
        make.size.mas_equalTo(CGSizeMake(25, 25));
    }];
    
    _psdfield2 = UITextField.new;
    _psdfield2.contentVerticalAlignment=UIControlContentVerticalAlignmentCenter;//设置其输入内容竖直居中
    CGRect frame4 = [_psdfield frame];
    frame4.size.width = 15.0f;
    UIView *leftview4 = [[UIView alloc] initWithFrame:frame4];
    _psdfield2.leftView=leftview4;//设置输入框内左边的图标
    [_psdfield2 setClearButtonMode:UITextFieldViewModeWhileEditing];//右侧删除按钮
    _psdfield2.leftViewMode=UITextFieldViewModeAlways;
    _psdfield2.placeholder=@"登录密码";//默认显示的字
    _psdfield2.secureTextEntry=YES;//设置成密码格式
    _psdfield2.keyboardType=UIKeyboardTypeDefault;//设置键盘类型为默认的
    _psdfield2.returnKeyType=UIReturnKeyDone;//返回键的类型
    _psdfield2.font = [UIFont systemFontOfSize:13.0f];
    _psdfield2.textColor = RGB(67, 67, 67);
    _psdfield2.delegate=self;//设置委托
    _psdfield2.tag = 10004;
    [self.view addSubview:_psdfield2];
    [_psdfield2 makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(lineview3.bottom);
        make.left.equalTo(psdicon.right);
        make.height.mas_equalTo(inputHeight);
        make.right.equalTo(superview.right).offset(-5);
    }];
    ...
    lineview4.backgroundColor = RGB(243, 183, 179);
    [self.view addSubview:lineview4];
    [lineview4 makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_psdfield2.bottom);
        make.left.equalTo(superview.left);
        make.right.equalTo(superview.right);
        make.height.mas_equalTo(0.5);
    }];
    ...
    [_regBtn setTitle:@"登录" forState:UIControlStateNormal];
    _regBtn.titleLabel.font = [UIFont systemFontOfSize:18.0f];
    [_regBtn addTarget:self action:@selector(logins) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:_regBtn];
    [_regBtn makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(lineview4.bottom).offset(28);
        make.left.equalTo(superview.left).offset(15);
        make.right.equalTo(superview.right).offset(-15);
